...
api.TabProperties.prototype = {
    /**
     * The maximum angle a tab can have if cutting a circle.
     */
    MAX_ANGLE : 45,
...
api.gcode.cutCircleWithTabs = function(
    ...
    str += feedrateString;
    code.push(str);
    code.push(api.gcode.selectXYPlane());
...
/**
 * Sets the plane in which the G2/G3 arcs are drawn on XY. Does not change the
 * fact that functions for cutting and pocketting the circles use XY plane. You
 * should not use this function and built-in functions for circle operation at
 * the same time.
 *
 * @return {string} The generated G-Code.
 */
api.gcode.selectXYPlane = function() {
    return "G17";
};

/**
 * Sets the plane in which the G2/G3 arcs are drawn on XZ. Does not change the
 * fact that functions for cutting and pocketting the circles use XY plane. You
 * should not use this function and built-in functions for circle operation at
 * the same time.
 *
 * @return {string} The generated G-Code.
 */
api.gcode.selectXZPlane = function() {
    return "G18";
};

/**
 * Sets the plane in which the G2/G3 arcs are drawn on YZ. Does not change the
 * fact that functions for cutting and pocketting the circles use XY plane. You
 * should not use this function and built-in functions for circle operation at
 * the same time.
 *
 * @return {string} The generated G-Code.
 */
api.gcode.selectYZPlane = function() {
    return "G19";
};
